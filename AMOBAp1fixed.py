...
def check():  # Checking win condition
    global gameend
#----------------------These are for X------------------------------
...
def pickingX():
    ...
    showtable()
    while True:        # repeating qiestion till correct answer
        try:
        ...
        while n >= 10 or n < 0:
            ...
            # checking if field is aready occuped
...
def pickingOM():
    ...
    while True:        # addig ismétli a kérdést amig teljesül a feltétel
        if(os.path.getmtime(fileName) > originalTime):
            with open(fileName, 'r') as f:
                ...
                break
            originalTime = os.path.getmtime(fileName)
        b = " Waiting for the answer" + "." * waiting_point + "   "
        print(b,end = "\r")
        time.sleep(3.5)
        if waiting_point < 3:
            waiting_point +=1
    ...
    print("                                                       ")
    print(R + "Enemy has choosen = " + str(n) + W)
